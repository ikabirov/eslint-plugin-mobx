...
function create(context) {
  ...
  return {
    'CallExpression[callee.name="makeObservable"]': (makeObservable) => {
      ...
      const unknownAnnotationProps = new Set(annotationProps.map(fieldToKey));
      ...
      members.forEach((member) => {
        ...
        if (
          !annotationProps.some(
            (prop) => fieldToKey(prop) === fieldToKey(member)
          )
        ) {
          // TODO optimize?
          nonAnnotatedMembers.push(member);
        } else {
          unknownAnnotationProps.delete(fieldToKey(member));
        }
      });
      /*
      ...
      */
      ...
      if (unknownAnnotationProps.size) {
        const keys = [...unknownAnnotationProps];
        const keyList = keys.map((key) => `\`${key}\``).join(", ");

        const fix = (fixer) => {
          return annotationProps
            .filter((prop) => unknownAnnotationProps.has(fieldToKey(prop)))
            .map((prop) => {
              return fixer.removeRange([prop.range[0], prop.range[1] + 2]);
            });
        };
        context.report({
          node: makeObservable,
...
module.exports = {
  meta: {
    ...
    messages: {
      ...
      redundantAnnotation: "Redundant annotation for {{ keyList }}.",
    },
  },
  create,
};
